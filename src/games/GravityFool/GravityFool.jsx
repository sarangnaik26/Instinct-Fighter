...
// A smaller impulse keeps the bird controllable while preserving the existing gravity.
const GRAVITY = 0.20
...
export default function GravityFool() {
  ...
  const initState = (W, H) => ({
    ...
    pipes: [], frameCount: 0, speed: PIPE_SPEED_INIT,
    lastTraumaFlip: Date.now(), nextTraumaDelay: randomBetween(2500, 7000), W, H,
  })
  ...
  const flap = useCallback(() => {
    ...
    sfx.jump()
    // Read the current gravity direction at input time. In particular, do not use
    // the normal direction for the first tap in Instinct mode (or after a Trauma flip).
  ...
  const startGame = useCallback(() => {
    ...
    // Establish the mode's gravity before the countdown ends, so the first tap
    // cannot race the first animation frame in reversed-gravity modes.
    const initiallyInverted = mode === 'instinct'
    invertedRef.current = initiallyInverted
    setIsInverted(initiallyInverted)
    const c = canvasRef.current
    if (c) stateRef.current = initState(c.offsetWidth, c.offsetHeight)
    setPhase('countdown')
  }, [mode])
    ...
    const loop = () => {
      ...
      if (phase === 'playing') {
        if (mode === 'trauma') {
          ...
          // Keep this ref authoritative for both input and physics in Instinct mode.
          invertedRef.current = mode === 'instinct'
        }
        ...
        if (absVy < -0.5) s.bird.angle = inv ? 25 : -25
        else if (absVy < 3.5) s.bird.angle += inv ? -1.5 : 1.5
        else s.bird.angle += inv ? -6 : 6
        ...
        const birdR = 13
        const died = s.bird.y < birdR || s.bird.y > H - birdR || s.pipes.some(p => {
      ...
      if (sprites.bg.complete && sprites.bg.naturalHeight !== 0) {
        ...
        for (let i = 0; i < tilesNeeded; i++) ctx.drawImage(sprites.bg, Math.floor(-shiftX + i * bgW), 0, Math.ceil(bgW) + 1, H)
      ...
      s.pipes.forEach(p => {
        ...
        if (woodReady) {
          ...
          ctx.fillStyle = '#2d9950'; ctx.fillRect(p.x, 0, PIPE_W, p.topH - 12); ctx.fillRect(p.x, botY + 12, PIPE_W, H - botY)
          ctx.fillStyle = '#1f7a38'; ctx.fillRect(p.x - 6, p.topH - 22, PIPE_W + 12, 22); ctx.fillRect(p.x - 6, botY, PIPE_W + 12, 22)
        }
      })

      ctx.fillStyle = '#8B6914'; ctx.fillRect(0, H - 22, W, 22); ctx.fillStyle = '#6B8E23'; ctx.fillRect(0, H - 26, W, 8)
      ctx.strokeStyle = '#00000066'; ctx.lineWidth = 2; ctx.strokeRect(0, H - 26, W, 26)
      ctx.fillStyle = '#8B6914'; ctx.fillRect(0, 0, W, 22); ctx.fillStyle = '#6B8E23'; ctx.fillRect(0, 18, W, 8); ctx.strokeRect(0, 0, W, 26)

      ctx.save(); ctx.translate(BIRD_X, s.bird.y); ctx.rotate((s.bird.angle * Math.PI) / 180)
      const birdSize = 38
      if (sprites.bird.complete && sprites.bird.naturalHeight !== 0) {
        ...
        let curFrame = 0
        const absVy = inv ? -s.bird.vy : s.bird.vy
        if (absVy < 3.5) curFrame = Math.floor(s.frameCount / 6) % numFrames
        ...
        ctx.fillStyle = '#FFE66D'; ctx.strokeStyle = '#000'; ctx.lineWidth = 2.5; ctx.beginPath(); ctx.ellipse(0, 0, 17, 13, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke()
        ctx.fillStyle = '#FFC200'; ctx.beginPath(); ctx.ellipse(-3, wingBob ? 5 : 2, 10, 5, wingBob ? -0.5 : 0.1, 0, Math.PI * 2); ctx.fill(); ctx.stroke()
        ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(7, -4, 5, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#111'; ctx.beginPath(); ctx.arc(8, -4, 3, 0, Math.PI * 2); ctx.fill()
        ctx.fillStyle = '#FF8C00'; ctx.beginPath(); ctx.moveTo(14, -1); ctx.lineTo(21, 2); ctx.lineTo(14, 5); ctx.closePath(); ctx.fill()
      }
      ctx.restore(); ctx.filter = 'none'

      if (phase === 'waiting') {
        ctx.fillStyle = 'rgba(0,0,0,0.45)'; ctx.fillRect(0, 0, W, H); ctx.fillStyle = '#fff'; ctx.font = 'bold 28px Fredoka One'; ctx.textAlign = 'center'; ctx.fillText('Tap to Start!', W / 2, H / 2 - 16); ctx.font = '18px Nunito'; ctx.fillStyle = 'rgba(255,255,255,0.7)'; ctx.fillText(game.desktopHint, W / 2, H / 2 + 18)
      }
      animRef.current = requestAnimationFrame(loop)
    }
    animRef.current = requestAnimationFrame(loop)
    return () => cancelAnimationFrame(animRef.current)
  }, [phase, mode, submitScore, game.desktopHint])

  useEffect(() => {
    const resize = () => { const c = canvasRef.current; if (!c) return; c.width = c.offsetWidth; c.height = c.offsetHeight; if (stateRef.current) { stateRef.current.W = c.width; stateRef.current.H = c.height } }
    resize(); window.addEventListener('resize', resize); return () => window.removeEventListener('resize', resize)
  }, [])

  return (
    <div className="relative w-full h-full bg-black" style={{ touchAction: 'none' }} onTouchStart={(e) => { e.preventDefault(); flap() }}>
      <canvas ref={canvasRef} className="w-full h-full" />
      <MuteButton />
      {(phase === 'playing' || phase === 'waiting') && <GameHUD score={score} mode={mode} gameId="gravity-fool" scoreUnit="" />}
      ...
      {phase === 'dead' && <GameOverScreen score={score} bestScore={bestScore} isNewRecord={isNewRecord} scoreUnit="" mode={mode} gameId="gravity-fool" onRetry={startGame} />}
    </div>
  )
}
